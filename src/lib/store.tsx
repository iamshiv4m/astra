...
function createDemoStore() {
  ...
  const persist = (state: DemoState) => {
    try {
      ...
      persistenceError =
        "In-memory demo mode: changes could not be saved in this browser and will be lost on reload. Allow browser storage or reset to retry.";
    }
  };
  ...
  const mutate = (operation: (state: DemoState) => DemoState) =>
    run(() => {
  ...
  const actions: DemoActions = {
    ...
    setScenario: scenario =>
      mutate(state => {
        if (!["normal", "loading", "error", "empty", "payment-failure"].includes(scenario))
    ...
    reset: () => {
      ...
      try {
        ...
        keys.forEach(key => {
          if (key?.startsWith("ASTRA:booking-draft:v1:")) localStorage.removeItem(key);
        });
      } catch {
        draftError =
          "Demo reset in memory, but old booking drafts could not be removed from browser storage. Allow storage and reset again before restoring a draft.";
      }
    ...
    clearError: () =>
      emit({
        error: storageBlocked ? "Saved demo data cannot be loaded. Use Reset demo to recover." : persistenceError,
      }),
  };
  return {
    ...
    hydrate: () => {
      ...
      if (raw !== null) {
        try {
          ...
          storageBlocked = true;
          emit({
            state: domain.createSeed(),
            ready: true,
            error:
              "Saved demo data is invalid or outdated. Your saved data was not overwritten. Use Reset demo to recover.",
          });
        }
      } else {
        commit(domain.createSeed());
    ...
    receive: (event: StorageEvent) => {
      ...
      try {
        if (event.newValue === null)
          throw new Error("The saved demo was removed in another tab. Reset demo to continue.");
...
export function DemoProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener("storage", store.receive);
    };
  }, [store]);
