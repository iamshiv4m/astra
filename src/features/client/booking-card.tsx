...
export function JoinAction({
  ...
  return (
    <Link className={className} href={`/session/${booking.sessionId}${mode === "demo" ? "?demo=1" : ""}`}>
  ...
  return (
    <article className={styles.bookingRow}>
      ...
      <div className={styles.bookingIdentity}>
        <h3>
          <Link href={`/dashboard/bookings/${booking.id}`}>{booking.astrologerName}</Link>
        </h3>
        <p>
      ...
      <Badge tone={booking.status === "completed" ? "muted" : booking.status === "active" ? "green" : "gold"}>
        {mode === "expired"
          ? "Time elapsed"
      ...
      <div className={styles.bookingActions}>
        ...
        <Link href={`/dashboard/bookings/${booking.id}`} className="btn btn-secondary">
