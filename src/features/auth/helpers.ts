...
export function validateIdentity({ name = "", contact, mode, signup = false }: {
  name?: string; contact: string; mode: "email" | "mobile"; signup?: boolean;
}): string | null {
  ...
  if (mode === "mobile" && !/^(?:91)?[6-9]\d{9}$/.test(contact.replace(/[\s()+-]/g, ""))) return "Enter a valid 10-digit Indian mobile number.";
  return null;
}
