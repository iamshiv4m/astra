  ...
  test(`complete client consultation journey at ${width}px`, async ({ page }) => {
    ...
    await test.step("Exercise all four steps and confirm one paid demo booking", async () => {
      ...
      await page
        ...
        .fill("Demo career direction — exploring a thoughtful next step.");
    ...
    await test.step("Download a genuine calendar event and revisit the booking", async () => {
      ...
      const nav = page.getByRole("navigation", {
        name: width < 850 ? "Quick navigation" : "client navigation",
        exact: true,
      });
    ...
    await test.step("Start the future demo without rescheduling and exchange a message", async () => {
      await page
        .getByRole("region", { name: "Your next consultation" })
        .getByRole("link", { name: "Start demo now", exact: true })
        .click();
    ...
    await test.step("End, rate, and find the completed session in booking history", async () => {
      ...
      await page
        .getByRole("textbox", { name: "A few words about your consultation (optional)", exact: true })
        .fill("A calm, useful demo conversation.");
      ...
      const completed = page
        .locator("article")
        .filter({ has: page.locator(`a[href="${bookingHref}"]`) })
        .first();
