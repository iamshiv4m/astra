...
describe("consultation entry", () => {
  const roomState = {
    ...
    astrologers: [
      { id: "ananya", name: "Ananya Sharma", title: "Vedic astrologer", image: "/portrait.svg", color: "#eee" },
    ],
  ...
  it("does not restart an active consultation after a refresh", () => {
    ...
    render(
      <ConsultationRoom
        ...
        state={{
          ...roomState,
          sessions: [{ ...roomState.sessions[0], status: "active", startedAt: "2026-09-19T08:59:00Z" }],
        }}
        actions={{ join } as unknown as DemoActions}
      />
    );
  ...
  it("cancels a pending simulated join when the room is left", () => {
    ...
    const view = render(
      <ConsultationRoom
        ...
        demoRequested
        state={roomState}
        actions={{ join, setCallStatus: vi.fn() } as unknown as DemoActions}
      />
    );
    fireEvent.click(screen.getByRole("button", { name: "Enter demo room" }));
    view.unmount();
  ...
  it("cancels pending scripted replies when the consultation ends", () => {
    ...
    const activeState = {
      ...roomState,
      sessions: [{ ...roomState.sessions[0], status: "active" as const, startedAt: "2026-09-19T08:59:00Z" }],
    };
    ...
    view.rerender(
      <ConsultationRoom
        ...
        state={{
          ...activeState,
          sessions: [{ ...activeState.sessions[0], status: "ended", endedAt: "2026-09-19T09:00:00Z" }],
        }}
        actions={actions}
      />
    );
  ...
  it("guards unknown and foreign sessions with a recovery link", () => {
    ...
    view.rerender(
      <ConsultationRoom sessionId="s1" state={{ ...roomState, clientId: "other" }} actions={{} as DemoActions} />
    );
  ...
  it("uses the shared main landmark rather than nesting another one", () => {
    render(
      <main id="main-content">
        <ConsultationRoom sessionId="s1" state={roomState} actions={{} as DemoActions} />
      </main>
    );
...
describe("consultation controls", () => {
  it("restores focus to chat when the mobile sheet closes", async () => {
    ...
    function SheetExample() {
      ...
      return (
        <ChatSheet open={open} onOpenChange={setOpen} onCloseFocus={restore}>
          <button onClick={() => setOpen(false)}>Back to call</button>
        </ChatSheet>
      );
    }
    render(<SheetExample />);
  ...
  it("exposes truthful simulated media states and persistent end action", () => {
    ...
    render(
      <CallControls
        session={{ muted: true, cameraOff: false, speakerOff: false, sharing: false } as Session}
        onToggle={toggle}
        onChat={() => {}}
        onEnd={end}
        chatOpen={false}
  ...
  it("requires a star rating and submits optional feedback", () => {
    ...
    fireEvent.change(screen.getByLabelText("A few words about your consultation (optional)"), {
      target: { value: "Thoughtful guidance." },
    });
    fireEvent.click(screen.getByRole("button", { name: "Save feedback" }));
...
describe("consultation room", () => {
  it("counts only consultation time and freezes at the real end", () => {
    ...
    expect(
      elapsedSeconds(
        { startedAt: "2026-09-19T10:00:00Z", endedAt: "2026-09-19T10:02:13Z" } as Session,
        Date.parse("2026-09-19T11:00:00Z")
      )
    ).toBe(133);
    expect(elapsedSeconds({ startedAt: "2026-09-19T11:00:00Z" } as Session, Date.parse("2026-09-19T10:00:00Z"))).toBe(
      0
    );
  });
  ...
  it("only allows matching participants and scheduled joining inside ten minutes", () => {
    expect(getSessionAccess(booking, "other", null, Date.parse("2026-09-19T09:55:00Z"))).toEqual({
      authorized: false,
      canJoin: false,
      roles: [],
    });
    ...
    expect(getSessionAccess(booking, "client-1", "ananya", Date.parse("2026-09-19T09:55:00Z")).roles).toEqual([
      "client",
      "astrologer",
    ]);
    expect(
      getSessionAccess({ ...booking, status: "completed" }, "client-1", null, Date.parse("2026-09-19T10:00:00Z"))
        .canJoin
    ).toBe(false);
  });
