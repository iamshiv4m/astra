...
const booking = (id: string, start: string, status: Booking["status"] = "confirmed", clientId = "shivam"): Booking => ({
  id,
  start,
  end: new Date(new Date(start).getTime() + 1800000).toISOString(),
  status,
  clientId,
  astrologerId: "ananya",
  ...
  it("uses the app shell main landmark without nesting another main", () => {
    const { container } = render(
      <main id="main-content">
        <DemoProvider>
          <BookingScreen astrologerId="ananya-sharma" query={{}} />
        </DemoProvider>
      </main>
    );
  ...
  it("keeps the same payment request when retrying and confirms only once", () => {
    ...
    const { rerender } = render(
      <BookingFlow
        ...
        state={state}
        actions={actions}
        query={{ date: state.seedDate, duration: "30", start: selected.start }}
      />
    );
    ...
    expect(screen.getByRole("link", { name: /View my booking/ })).toHaveAttribute(
      "href",
      expect.stringContaining("/dashboard/bookings/booking-request-")
    );
  ...
  it("requires sign-in without dropping date, duration, and selected time", () => {
    ...
    render(
      <BookingFlow
        ...
        query={{ date: state.seedDate, duration: "45", start: selected.start }}
      />
    );
    const url = new URL(
      screen.getByRole("link", { name: /Sign in to confirm/ }).getAttribute("href")!,
      "https://astra.test"
    );
  ...
  it("does not submit a stale time when shared availability changes", () => {
    ...
    const { rerender } = render(
      <BookingFlow
        ...
        state={state}
        actions={actions}
        query={{ date: state.seedDate, start: selected.start }}
      />
    );
    const occupied = bookConsultation(state, {
      astrologerId: advisor.id,
      start: selected.start,
      duration: 30,
      requestId: "another-tab",
    }).state;
    rerender(
      <BookingFlow
        astrologer={advisor}
  ...
  it("does not claim a persistent profile save when the store reports a storage warning", () => {
    ...
    rerender(
      <ProfileForm
        ...
        today="2026-09-19"
        onSave={() => {}}
        saveWarning="Changes were not saved to this browser."
      />
    );
    expect(screen.queryByText("Your profile has been saved.")).not.toBeInTheDocument();
  });
...
describe("booking intent", () => {
  it("restores an explicit profile choice before a saved draft", () => {
    const draft = restoreDraft(
      "ananya",
      { duration: "45", date: "2026-09-19", start: slot.start },
      JSON.stringify({
        version: 1,
        astrologerId: "ananya",
        duration: 60,
...
describe("client workspace", () => {
  it("derives owned upcoming and past records from one booking source", () => {
    const records = [
      booking("2", "2026-09-20T10:00:00Z"),
      booking("1", "2026-09-19T10:00:00Z"),
      booking("old", "2026-09-18T10:00:00Z", "completed"),
      booking("foreign", "2026-09-19T10:00:00Z", "confirmed", "other"),
    ];
