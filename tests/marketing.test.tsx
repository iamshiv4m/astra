...
import { QuestionChapter, ConversationPath, storyTopics } from "@/features/story/story-home";
...
it("connects Indian traditions and a preferred language to matching guides", () => {
  ...
  expect(screen.getByRole("link", { name: /Explore Vedic Astrology/ })).toHaveAttribute("href", "/astrologers?specialty=Vedic+Astrology&language=Tamil");
  expect(screen.getByRole("link", { name: /See Tamil-speaking guides/ })).toHaveAttribute("href", "/astrologers?language=Tamil");
...
it("lets visitors choose the question that starts their story", () => {
  ...
  expect(screen.getByRole("link", { name: /Find guidance for relationships/i })).toHaveAttribute("href", "/astrologers?search=Relationships");
