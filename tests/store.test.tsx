...
function Probe() {
  const demo = useDemo();
  useEffect(() => { actions = demo.actions; }, [demo.actions]);
  return <div><span data-testid="ready">{String(demo.ready)}</span><span data-testid="identity">{demo.state.clientId ?? "guest"}</span><span data-testid="clock">{demo.state.now}</span><span data-testid="error">{demo.error}</span><span data-testid="bookings">{demo.state.bookings.length}</span></div>;
}
...
describe("persistent demo provider", () => {
  it("keeps the server snapshot stable while a suspended descendant hydrates after the provider", async () => {
    ...
    const app = <DemoProvider><Suspense fallback={<span>Loading boundary</span>}><DelayedProbe /></Suspense></DemoProvider>;
