...
At 600px and below, the homepage is a concise decision path rather than the desktop story stacked vertically. Keep its default document height at or below 3100px for the seeded demo at 375/390/430/572px widths; the hero stays below 520px and guide discovery starts within 1100px.

Open with the illustrative kundli, a short offer and a primary action. Remove the competing question card, language pitch, moving voice ribbon and chapter navigation on phones. Combine topic selection and consultation language into one discovery area with a matching-guide count and a single directory handoff; a selected topic can be deselected, and empty combinations explicitly offer changing choices or browsing all guides. Astrology approaches remain available on request.

Keep all four profile rows with rating/price/profile links. Replace the phone's duplicate consultation preview with three plain steps: Choose, Book, Connect. Show one clearly illustrative testimonial, one closing action and a compact homepage footer. Keep demo disclosures and recovery/privacy links visible; remove the promotional trust ribbon on phones rather than shrinking its caveats. Desktop retains the complete editorial story and interactive preview. Never shrink touch targets or clamp essential booking information to meet the height budget.
